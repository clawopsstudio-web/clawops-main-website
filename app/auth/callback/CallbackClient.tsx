  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      try {
        ...
        log(`3. getSession: session=${!!session}, error=${error?.message ?? 'none'}`)
        ...
        if (session) {
          ...
          log('4. No session — checking URL params')
          const params = new URLSearchParams(window.location.search)
          log(`   URL search: ${window.location.search}`)
          if (params.has('code')) {
            log('   Code found — trying explicit exchange')
          ...
          log('5. No session found — redirecting to login')
